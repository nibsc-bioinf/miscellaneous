#This is a script to write a bash script with qsub commands to loop running Kraken over all fastq.gz files in a given directory
#Usage:
...
#/usr/local/bin/kraken-mpa-report --db /usr/local/share/kraken/database/ $outdir/kraken.output >  $outdir/kraken.report
#Then qsub that script with a qsub hold until all the previous ones are finished
